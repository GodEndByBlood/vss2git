...
bool file::CreateDirectoryRecursive(const CString& sDirName, LPSECURITY_ATTRIBUTES lpSecurityAttributes)
{
	...
	int nIdx = sDirName.Find(L":");
	if(nIdx == -1)
	{
		nIdx = sDirName.Find(L"\\\\");
		if(nIdx != -1)
